...
Ext.ux.ColorPicker = Ext.extend(Ext.BoxComponent, {
    ...
    onRender: function () {
        ...
        if (!this.el) {
            this.el = this.body;
            if (this.cls) {
                Ext.get(this.el).addClass(this.cls);
            }
        }
    ...
    renderComponent: function () {
        ...
        Ext.DomHelper.append(this.cpGetId('cCont'), {
            'cls': 'x-btn x-btn-small x-cp-colorbox x-cp-websafe',
            'id': this.cpGetId('cWebSafe')
        }, true).update('Websafe');
        Ext.DomHelper.append(this.cpGetId('cCont'), {
            'cls': 'x-btn x-btn-small x-cp-colorbox x-cp-inverse',
            'id': this.cpGetId('cInverse')
        }, true).update('Inverse');
        Ext.DomHelper.append(this.cpGetId('cCont'), {
            'cls': 'x-btn x-btn-small x-cp-colorbox x-cp-pickcolor',
            'id': this.cpGetId('cColor')
        }, true).update('Pick Color');
     ...
     *
     */
    updateFromIHSV: function () {
        this.updateMode = 'hsv';
        this.HSV = {
            h: Ext.getCmp(this.cpGetId('iHue')).getValue(),
            s: Ext.getCmp(this.cpGetId('iSat')).getValue() / 100,
            v: Ext.getCmp(this.cpGetId('iVal')).getValue() / 100
        };
        this.updateColor();
    },
    /**
